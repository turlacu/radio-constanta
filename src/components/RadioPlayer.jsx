...
export default function RadioPlayer({ radioState }) {
  ...
  // Responsive layout classes - use mobile styling for split-screen too
  ...
  return (
    <div className={containerClasses}>
      ...
      {/* Blurred gradient background - prominent in split-screen */}
      <div className={`absolute inset-0 ${isSplitScreen ? 'opacity-100' : 'opacity-30'}`}>
        <motion.div
          animate={{
            background: isSplitScreen ? [
        ...
        />
        {isSplitScreen && (
          <div className="absolute inset-0 backdrop-blur-2xl bg-dark-bg/30" />
        )}
      </div>
      ...
      <motion.div
        ...
        className={`relative w-full mb-6 ${
          isSplitScreen
            ? 'max-w-[340px]' // Keep mobile size in split-screen
            : 'max-w-[340px] md:max-w-[400px] md:mb-8 lg:max-w-[480px] lg:mb-10 tv:max-w-[600px] tv:mb-0 tv:flex-shrink-0'
        }`}
      >
        {/* Outer glow ring */}
        <motion.div
      ...
      <div className={`flex flex-col items-center w-full relative z-10 ${
        isSplitScreen ? '' : 'tv:flex-1 tv:items-start'
      }`}>
        {/* Station Info */}
        <motion.div
          initial={{ y: 20, opacity: 0 }}
          ...
          className={`text-center mb-6 ${
            isSplitScreen ? '' : 'md:mb-8 tv:text-left tv:mb-10'
          }`}
        >
          <h2 className={`font-bold mb-2 bg-gradient-to-r from-white to-white/80 bg-clip-text text-transparent ${
            isSplitScreen
              ? 'text-2xl' // Keep mobile size in split-screen
          ...
          {metadata && (
            <motion.p
              ...
              className={`text-white/70 font-medium ${
                isSplitScreen
                  ? 'text-sm' // Keep mobile size in split-screen
        ...
        <motion.button
          ...
          disabled={isLoading}
          tabIndex={0}
          className={`relative rounded-full disabled:opacity-50 group tv-focusable mb-6 ${
            isSplitScreen
              ? 'w-16 h-16' // Keep mobile size in split-screen
              : 'w-16 h-16 md:w-20 md:h-20 md:mb-8 lg:w-24 lg:h-24 tv:w-32 tv:h-32 tv:mb-12'
          }`}
        >
        ...
        <div className={`relative w-full h-full rounded-full backdrop-blur-xl border-2 border-white/30 shadow-2xl flex items-center justify-center group-hover:border-white/50 transition-colors ${
          ...
          {isLoading ? (
            ...
            <svg className={`text-white drop-shadow-lg ${
              isSplitScreen
                ? 'w-8 h-8' // Keep mobile size in split-screen
                : 'w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 tv:w-16 tv:h-16'
            }`} fill="currentColor" viewBox="0 0 24 24">
              <path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z" />
            </svg>
          ) : (
            <svg className={`text-white ml-1 drop-shadow-lg ${
              isSplitScreen
                ? 'w-8 h-8' // Keep mobile size in split-screen
                : 'w-8 h-8 md:w-10 md:h-10 md:ml-1 lg:w-12 lg:h-12 lg:ml-2 tv:w-16 tv:h-16 tv:ml-3'
            }`} fill="currentColor" viewBox="0 0 24 24">
        ...
        <motion.div
          ...
          className={`flex gap-2 w-full mb-5 ${
            isSplitScreen
              ? 'max-w-xs' // Keep mobile size in split-screen
              : 'max-w-xs md:gap-3 md:max-w-sm md:mb-6 lg:max-w-md lg:gap-4 tv:max-w-2xl tv:gap-6 tv:mb-8'
          }`}
        >
        {stations.map((station, index) => (
          <motion.button
            ...
            className={`
              ...
              ${isSplitScreen
                ? 'py-3 px-3 text-xs' // Keep mobile size in split-screen
                : 'py-3 px-3 text-xs md:py-4 md:px-4 md:text-sm lg:py-5 lg:px-5 lg:text-base tv:py-6 tv:px-8 tv:text-2xl'
              }
        ...
        {availableQualities.map((quality, index) => (
          <motion.button
            ...
            className={`
              ...
              ${isSplitScreen
                ? 'py-2 px-2 text-xs' // Keep mobile size in split-screen
                : 'py-2 px-2 text-xs md:py-2.5 md:px-3 md:text-sm lg:py-3 lg:px-4 lg:text-base tv:py-4 tv:px-6 tv:text-xl'
              }
        ...
        {streamInfo && (
          <motion.div
            ...
            className={`flex items-center justify-center gap-2 text-primary font-medium ${
              isSplitScreen
                ? 'text-xs' // Keep mobile size in split-screen
                : 'text-xs md:gap-3 md:text-sm lg:text-base lg:gap-4 tv:text-xl tv:gap-6'
            }`}
          >
            <span>{streamInfo.format}</span>
            <span className={`bg-white/30 rounded-full ${isSplitScreen ? 'w-1 h-1' : 'w-1 h-1 tv:w-2 tv:h-2'}`} />
